...
if ( ! class_exists( 'YITH_WCStripe_Gateway' ) ) {
	...
	class YITH_WCStripe_Gateway extends WC_Payment_Gateway_CC {
		...
		public function payment_scripts() {
			...
			wp_register_script( 'yith-stripe-js', YITH_WCSTRIPE_URL . 'assets/js/stripe-checkout' . $suffix . '.js', array( 'jquery', 'jquery-blockui', 'stripe-js' ), YITH_WCSTRIPE_VERSION, true );
					...
					foreach ( $items as $item ) {
						...
						if ( $item->is_type( 'line_item' ) ) {
							...
							if ( $product->get_short_description() ) {
								$line_item['description'] = strip_tags( $product->get_short_description() );
							}
